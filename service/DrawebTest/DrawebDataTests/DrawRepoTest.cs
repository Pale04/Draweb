...
public class DrawRepoTest : IClassFixture<TestDatabaseFixture>
{
    ...
    {
        var repo = new DrawingRepo(Fixture.CreateContext());
        int userId = 1;
    ...
    {
        var repository = new DrawingRepo(Fixture.CreateContext());
    ...
    public async Task DeleteDraw_ShouldReturnSuccessfulResult()
    {
        ...
        var result = await repository.DeleteDrawing(drawId);
    ...
    public async Task UpdateDraw_ShouldReturnSuccessfulResult()
    {
        ...
        var repository = new DrawingRepo(context);
        ...
        DateTime? previousUpdateTime = userDraws.Data!.First(d => d.DrawingId == drawId).LastUpdate;
        ...
        DateTime? newUpdateTime = updatedUserDraws.Data!.First(d => d.DrawingId == drawId).LastUpdate;

        Assert.True(result.IsSuccess);
        Assert.NotNull(result.Data);
        Assert.Equal(updatedUserDraws.Data!.First(d => d.DrawingId == drawId).Title, title);
